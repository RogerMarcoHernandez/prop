...
/**
 * Clase variante Codemaker de Partida.
 * @author Roger Marco
 *
 */
public class PartidaCodemaker extends Partida implements IPartida, Serializable {
	private static final long serialVersionUID = 1L;
	
	/**
	 *  Instancia de maquina.
	 */
	transient private Maquina maquina;

	/**
	 *  Metodo para inicializar la maquina genetic de acuerdo a la configuracion de la partida.
	 */
	private void configuraMaquinaGenetic() {
	...
	/**
	 *  Metodo para inicializar la maquina five guess de acuerdo a la configuracion de la partida.
	 */
	private void configuraMaquinaFiveGuess() {
		this.maquina = new MaquinaFiveGuess(obtenNumeroColores(), obtenNumeroTurnos(), obtenLongitudCodigo());
	}
	...
	public PartidaCodemaker(int id, List<Object> config, Perfil usuarioActivo) throws Exception {
		...
		if (obtenDificultad() < 3) configuraMaquinaGenetic();
		else configuraMaquinaFiveGuess();
	}
						...
						if (codigoSecreto == null)
							...
							if (!turnosFinalizados || !acabada) {
								if(obtenAdivinanza()==null || obtenAdivinanza().isEmpty()) {
									...
									List<List<Integer>> codigosMaquina = maquina.solve(codigoSecreto);
									// Instancia para obtener el ultimo codigo de la lista (el mejor).
									List<Integer> mejorCodigoMaquina = codigosMaquina.get(obtenTurnoActual());
	...
	private void sincronizarConfiguracionGenetic() {
		if(ConfiguracionGenetic.obtenLongitudCodigo()!=obtenLongitudCodigo() || 
				...
				ConfiguracionGenetic.obtenDificultad()!=obtenDificultad()) configuraMaquinaGenetic();
	}
